...
def setrun(claw_pkg='geoclaw'):
	...
	# 1. Magueyes Island, PR,                                Station ID: 9759110,      17.9700 N, 67.0465 W        Landfall near Dominican Republic
    ...
    regions.append([1,3,rundata.clawdata.t0, rundata.clawdata.tfinal, -100   , -60   , 10   , 45   ]) # keep non-gauges up to 3
    regions.append([4,6,rundata.clawdata.t0, rundata.clawdata.tfinal, -67.25, -66.75, 17.75, 18.25]) # Gauge 1
    regions.append([5,6,rundata.clawdata.t0, rundata.clawdata.tfinal, -82   , -81.5 , 24.25, 24.75]) # Gauge 2
    regions.append([6,6,rundata.clawdata.t0, rundata.clawdata.tfinal, -83.25, -82.75, 29   , 29.5 ]) # Gauge 3
    regions.append([5,6,rundata.clawdata.t0, rundata.clawdata.tfinal, -91.75, -91.25, 29   , 29.5 ]) # Gauge 4
    regions.append([6,6,rundata.clawdata.t0, rundata.clawdata.tfinal, -94   , -93.5 , 29.25, 30   ]) # Gauge 5
...
def setgeo(rundata):
    ...
    # See regions for control over these regions, need better bathy data for the smaller domain 
    ...
    Laura = Storm(path=atcf_path, file_format="ATCF")

    # Calculate landfall time - Need to specify as the file above does not
    # include this info (08/23/2020 ~ 0430 UTC), can't do float so round to nearest integer 0400 UTC
    Laura.time_offset = datetime.datetime(2020, 8, 27, 6)

    Laura.write(data.storm_file, file_format='geoclaw')
