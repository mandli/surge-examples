...
subroutine bc2amr(val,aux,nrow,ncol,meqn,naux, hx, hy, level, time,   &
    ...
    if (xlo_patch < xlower-hxmarg) then
        ...
        select case(mthbc(1))
            case(0) 
                ! User defined boundary condition - decrease incoming momentum
                do j = 1, ncol
                    do i=1, nxl
                        aux(:, i, j) = aux(:, nxl + 1, j)
                        val(1, i, j) = val(1, nxl + 1, j)

                        if (val(2, nxl + 1, j) < 0.d0) then ! Outgoing
                            val(2, i, j) = val(2, nxl + 1, j)
                        else ! Incoming
                            val(2, i, j) = alpha_bc * val(2, nxl + 1, j)
                        end if
            ...
            case(0)
                ...
                do i = ibeg, nrow
                    do j = 1, ncol
                        ...
                        if (val(2, ibeg - 1, j) > 0.d0) then ! Outgoing
                            val(2, i, j) = val(2, ibeg - 1, j)
                        else ! Incoming
                            val(2, i, j) = alpha_bc * val(2, ibeg - 1, j)
                        end if
