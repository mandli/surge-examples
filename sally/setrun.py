...
def setrun(claw_pkg='geoclaw'):
    ...
    # Range of topo file is -96 -70 20 37
    # Lower and upper edge of computational domain:
    clawdata.lower[0] = -95      # west longitude
    clawdata.upper[0] = -71      # east longitude

    clawdata.lower[1] = 21       # south latitude
    clawdata.upper[1] = 36       # north latitude
    ...
    # Initial time:
    # -------------
    clawdata.t0 = days2seconds(-4.5) # 4.5 days before landfall
    ...
    if clawdata.output_style == 1:
        ...
        clawdata.tfinal = days2seconds(2) # 2 days after landfall
    ...
    regions.append([1, 2, clawdata.t0, clawdata.tfinal, -95, -71, 32, 36]) # Top
    regions.append([1, 2, clawdata.t0, clawdata.tfinal, -95, -94, 21, 36]) # Left
    regions.append([1, 2, clawdata.t0, clawdata.tfinal, -78, -71, 21, 36]) # Right
    regions.append([5, 5, clawdata.t0, clawdata.tfinal, -90, -89.3, 29.7, 29.9]) # For Gauge 9
    regions.append([5, 5, clawdata.t0, clawdata.tfinal, -89.4, -89.2, 30.2, 30.4]) # For Gauge 10

    # Topography file from scratch directory
    ...
    topo_file = topo_file.crop((-95.5, -70.5, 20.5, 36.5))


    # The following code has been used in other simulations to select additional
    # regions for refinement.
    ...
    lat = all_gauges.iloc[:,5] # latitude column
    long = all_gauges.iloc[:,8] # longitude columns

    lat_long = [[lat[i], long[i]] for i in range(lat.size)]
    num_gauges = len(lat_long) # number of gauges

    for i in range(num_gauges):
        rundata.gaugedata.gauges.append([i+1, lat_long[i][1] # longitude
                                            , lat_long[i][0], # latitude
                                       rundata.clawdata.t0,
                                       rundata.clawdata.tfinal])
    
    # To manually append gauges (without .csv file), use:
    #
    # rundata.gaugedata.gauges.append([number, longitude, latitude,
    #                                    rundata.clawdata.t0,
    #                                    rundata.clawdata.tfinal])
    # Note: number is the integer number of the gauge (1, 2, 3, ...)
...
def setgeo(rundata):
    ...
    sally.write(data.storm_file, file_format='geoclaw')
