# Validation for Hurricane Elsa by Max Zhao, New Workflow
"""
...
#======================
storm_num = 'al052021'
landfall = datetime.datetime(2021, 7, 7, 14)
bottom_left = (-95.0, 12.0)
top_right = (-70.0, 32.0)
#======================
#======================

storm = auto_analysis.generate_storm_data(storm_num)

location = []
for i in range(len(storm)):
    lat_raw = re.findall(r'\d+',storm[i][6])
    lon_raw = re.findall(r'\d+',storm[i][7])
    time_raw = re.findall(r'\d+',storm[i][2])
    lat = float(int(lat_raw[0])/10)
    lon = float(int(lon_raw[0])/10)
    ti = datetime.datetime(int(time_raw[0][0:4]), int(time_raw[0][4:6]), int(time_raw[0][6:8]), int(time_raw[0][8:]))
    location.append((lat, -1*lon, ti))

def find_time(bl, tr, p) :
...
for i in location:
    point = (i[1], i[0])
    if find_time(bottom_left, top_right, point) == True:
        valid.append(i[2])
if float((valid[-1] - landfall).total_seconds()/86400) < 1:
    after = math.ceil(float((valid[-1] - landfall).total_seconds()/86400))
else:
    after = math.floor(float((valid[-1] - landfall).total_seconds()/86400))
if float((landfall - valid[0]).total_seconds()/86400) < 1:
    before = math.ceil(float((landfall - valid[0]).total_seconds()/86400))
else:
    before = math.floor(float((landfall - valid[0]).total_seconds()/86400))
...
def setrun(claw_pkg='geoclaw'):
    ...
    clawdata.lower[0] = bottom_left[0]     # west longitude 90
    clawdata.upper[0] = top_right[0]     # east longitude 70

    clawdata.lower[1] = bottom_left[1]      # south latitude 18
    clawdata.upper[1] = top_right[1]      # north latitude  32
    ...
    # Initial time:
    # -------------
    clawdata.t0 = -days2seconds(before)
    ...
    if clawdata.output_style == 1:
        ...
        clawdata.tfinal = days2seconds(after)
        recurrence = 4
...
def setgeo(rundata):
    ...
    auto = Storm(path=atcf_path, file_format="ATCF")

    # Calculate landfall time - Need to specify as the file above does not
    # include this info (8/26/2017 ~4am UTC)
    auto.time_offset = landfall

    auto.write(data.storm_file, file_format='geoclaw')
