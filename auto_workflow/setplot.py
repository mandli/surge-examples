...
gauge = auto_analysis.generate_gauge(meta, setrun.storm)
copy = gauge.copy()
for i in copy:
    if copy[i][1]>setrun.top_right[1] or copy[i][1]<setrun.bottom_left[1] or copy[i][2]<setrun.bottom_left[0] or copy[i][2]>setrun.top_right[0]:
        gauge.pop(i)
lat = []
lng = []
for item in gauge:
    lat.append(gauge[item][1])
    lng.append(gauge[item][2])
...
def setplot(plotdata=None):
    ...
    plotaxes = plotfigure.new_plotaxes()
    plotaxes.xlimits = [-setrun.before, setrun.after]
    ...
    count = []
    for item in gauge:
        my_dict[len(count)+1] = gauge[item][0]
        count.append(item)
    
    def gauge_afteraxes(cd): 
        t0 = setrun.landfall-timedelta(days=setrun.before)
        t_offset = setrun.landfall
        tf = setrun.landfall+timedelta(days=setrun.after)
        ...
        axes.set_xlim([-setrun.before, setrun.after]) # change to t_0 and t_1
        axes.set_ylim([-1, 2])
    ...
    plotaxes.scaled = True
    plotaxes.xlimits = [min(lng)-2, max(lng)+2]
    plotaxes.ylimits = [min(lat)-1, max(lat)+1]
