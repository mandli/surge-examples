...
def setrun(claw_pkg='geoclaw'):
    ...
    # Range of topo file is -100 -55 12 45
    ...
    # Initial time:
    # -------------
    clawdata.t0 = days2seconds(-0.75) # 0.75 days before landfall
    ...
    if clawdata.output_style == 1:
        ...
        clawdata.tfinal = days2seconds(1.5) # 1.5 days after landfall
    ...
    clawdata.verbosity = 2 # Change to 2 (instead of 0)
    ...
    regions.append([4, 5, clawdata.t0, clawdata.tfinal, -94, -85, 28, 32])
    
    # Topography file from scratch directory
    topo_path = os.path.join(scratch_dir, "gebco_2022_n45.0_s12.0_w-100.0_e-55.0.asc")
    topo_file = topo.Topography()
    topo_file.read(topo_path, topo_type=3)
    topo_file = topo_file.crop((-99.5, -70.5, 12.5, 41.5))

    # The following code has been used in other simulations to select additional regions for refinement
    ...
    long = all_gauges.iloc[:,8] # Longitude Column

    lat_long = [[lat[i], long[i]] for i in range(lat.size)]
    num_gauges = len(lat_long) # Number of gauges

    for i in range(num_gauges):
        rundata.gaugedata.gauges.append([i+1, lat_long[i][1], lat_long[i][0], # Longitude, Latitude
                                       rundata.clawdata.t0,  
...
def setgeo(rundata):
    ...
    clawutil.data.get_remote_file(
                   "https://ftp.nhc.noaa.gov/atcf/archive/2020/bal282020.dat.gz ")
    ...
    # include this info (10/28/2020 ~9pm UTC)


    # Time of Landfall
    zeta.time_offset = datetime.datetime(2020, 10, 28, 21) 

    zeta.write(data.storm_file, file_format='geoclaw')
